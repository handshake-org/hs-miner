...
const os = require('os');
const binding = require('bindings')('hskminer');
const miner = exports;
...
miner.toBuffer = function toBuffer(arr) {
  ...
  const buf = new Buffer(arr.length << 2);

  let j = 0;

  for (let i = 0; i < arr.length; i++) {
    buf.writeUInt32LE(arr[i], j, true);
    j += 4;
  }
