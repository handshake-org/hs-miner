...
#endif
#define EASINESS \
  ((uint32_t)(((uint64_t)PERC * (NEDGES * 2)) / 100))

// generate edge endpoint in cuckoo graph without partition bit
static edge_t _sipnode(siphash_keys *keys, edge_t nonce, u32 uorv) {
  return siphash24(keys, 2*nonce + uorv) & EDGEMASK;
}

// generate edge endpoint in cuckoo graph
static node_t sipnode(siphash_keys *keys, edge_t nonce, u32 uorv) {
...
static int verify(edge_t nonces[PROOFSIZE], siphash_keys *keys) {
