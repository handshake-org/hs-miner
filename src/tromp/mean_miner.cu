...
typedef meancu_zbucket<ZBUCKETSIZE, NZ1, NZ2> bigbuck;
typedef bigbuck bigbucks[NY];

typedef meancu_zbucket<Z2BUCKETSIZE, 0, 0> smallbuck;

struct meancu_indexer {
  bigbucks *buckets;
  u32 index[NX];
...
class meancu_edgetrimmer; // avoid circular references
...
struct meancu_twostagetpb {
  u16 stage1tpb;
  u16 stage2tpb;
};

struct meancu_trimparams {
  u16 nblocks;
  u16 ntrims;
  u16 genUblocks;
  u16 genUtpb;
  meancu_twostagetpb genV;
  meancu_twostagetpb trim;
  meancu_twostagetpb rename[2];
  u16 trim3tpb;
  u16 rename3tpb;
  u16 reportcount;
  u16 reportrounds;
...
struct meancu_edgetrimmer {
  meancu_trimparams tp;
  siphash_keys sip_keys;
  meancu_edgetrimmer *dt;
  bigbucks *buckets;
  bigbucks *tbuckets;
  zbucket32 *tnames;
  u32 *uvnodes;
  ...
  meancu_edgetrimmer(const meancu_trimparams _tp) {
    tp = _tp;
    checkCudaErrors(cudaMalloc((void**)&dt, sizeof(meancu_edgetrimmer)));
...
// write         edge     UZZZZ   within UX UY partition
  ...
  __device__ void genVnodes2(const u32 part, const u32 uorv) {
    ...
    __shared__ meancu_indexer dst;
    __shared__ meancu_twice_set<NZ> degs;
...
// write      UXXXXX    UYYYYY     UZZZZZ     VZZZZ   within VX VY partition
      ...
      { DO_NOTHING("OOPS3: id %d vx %d ux %d uyz %x\n", blockIdx.x, vx, ux, uyz); break; }
    }
    __syncthreads();
    dst.storeu(blockIdx.x);
  }
...
// write     VYYZZ'     UYYYYY     UZZZZ   within UX partition  if TRIMONV
  ...
  __device__ void trimedges3(const u32 round) {
    __shared__ meancu_twice_set<NYZ1> degs;
...
// write    UXXYYZZ"  VXXYYZZ"
  ...
  template <u32 SRCSIZE, u32 DSTSIZE, bool TRIMONV> void _trimedges(meancu_edgetrimmer *et, const u32 round);
  template <u32 SRCSIZE, u32 DSTSIZE, bool TRIMONV> void _trimrename(meancu_edgetrimmer *et, const u32 round);

  void trim();
};

__global__ void _genUnodes(meancu_edgetrimmer *et, const u32 uorv) {
  et->genUnodes(uorv);
}

__global__ void _genVnodes1(meancu_edgetrimmer *et, const u32 part) {
  et->genVnodes1(part);
}

__global__ void _genVnodes2(meancu_edgetrimmer *et, const u32 part, const u32 uorv) {
  et->genVnodes2(part, uorv);
}

template <u32 SRCSIZE, u32 DSTSIZE, bool TRIMONV>
__global__ void _trimedges1(meancu_edgetrimmer *et, const u32 round, const u32 part) {
  et->trimedges1<SRCSIZE, DSTSIZE, TRIMONV>(round, part);
}

template <u32 DSTSIZE, bool TRIMONV>
__global__ void _trimedges2(meancu_edgetrimmer *et, const u32 round, const u32 part) {
  et->trimedges2<DSTSIZE, TRIMONV>(round, part);
}

template <u32 SRCSIZE, u32 DSTSIZE, bool TRIMONV>
void meancu_edgetrimmer::_trimedges(meancu_edgetrimmer *et, const u32 round) {
...
template <u32 SRCSIZE, u32 DSTSIZE, bool TRIMONV>
__global__ void _trimrename1(meancu_edgetrimmer *et, const u32 round, const u32 part) {
  et->trimrename1<SRCSIZE, DSTSIZE, TRIMONV>(round, part);
}

template <u32 SRCSIZE, u32 DSTSIZE, bool TRIMONV>
__global__ void _trimrename2(meancu_edgetrimmer *et, const u32 round, const u32 part) {
  et->trimrename2<SRCSIZE, DSTSIZE, TRIMONV>(round, part);
}

template <u32 SRCSIZE, u32 DSTSIZE, bool TRIMONV>
void meancu_edgetrimmer::_trimrename(meancu_edgetrimmer *et, const u32 round) {
  for (u32 part=0; part < NX/tp.nblocks; part++) {
...
template <bool TRIMONV>
__global__ void _trimedges3(meancu_edgetrimmer *et, const u32 round) {
  et->trimedges3<TRIMONV>(round);
}

template <bool TRIMONV>
__global__ void _trimrename3(meancu_edgetrimmer *et, const u32 round) {
  et->trimrename3<TRIMONV>(round);
}

__global__ void _recoveredges(meancu_edgetrimmer *et) {
  et->recoveredges();
}

__global__ void _recoveredges1(meancu_edgetrimmer *et) {
  et->recoveredges1();
}
...
#define BIGGERSIZE BIGSIZE+1
  ...
  void meancu_edgetrimmer::trim() {
    cudaMemcpy(dt, this, sizeof(meancu_edgetrimmer), cudaMemcpyHostToDevice);
...
struct meancu_solver_ctx {
  meancu_edgetrimmer *trimmer;
  smallbuck *buckets;
  u32 *cuckoo;
  ...
  meancu_solver_ctx(const meancu_trimparams tp) {
    trimmer = new meancu_edgetrimmer(tp);
