...
template<u32 BUCKETSIZE>
using yzbucket = mean_zbucket<BUCKETSIZE>[NY];
template <u32 BUCKETSIZE>
...
class mean_edgetrimmer; // avoid circular references
...
public:
  ...
  void* operator new(size_t size) noexcept {
    void* newobj;
    int tmp = posix_memalign(&newobj, NSIPHASH * sizeof(u32), sizeof(mean_edgetrimmer));
  ...
  mean_edgetrimmer(const u32 n_threads, const u32 n_trims, const bool show_all) {
...
#error not implemented
    ...
    rdtsc1 = __rdtsc();
    if (!id) DO_NOTHING("genUnodes round %2d size %u rdtsc: %lu\n", uorv, sumsize/BIGSIZE0, rdtsc1-rdtsc0);
  ...
  void genVnodes(const u32 id, const u32 uorv) {
    ...
    mean_indexer<ZBUCKETSIZE> dst;
    mean_indexer<TBUCKETSIZE> small;
...
// if (id==1) DO_NOTHING("id %d ux %d y %d size %u sumsize %u\n", id, ux, uy, tbuckets[id][uy].size/BIGSIZE, sumsize);
...
// if (id==0) DO_NOTHING("id %d ux %d uy %d e %010lx pref %4x edge %x mask %x\n", id, ux, uy, e, e>>ZBITS, edge, NONDEGMASK);
          *edges = edge;
          const u32 z = e & ZMASK;
          *zs = z;
        ...
        { DO_NOTHING("OOPS2: id %d ux %d uy %d edge %x vs %x\n", id, ux, uy, edge, EDGEMASK); exit(0); }
...
// DO_NOTHING("id %d ux %d y %d edge %08x e' %010lx vx %d\n", id, ux, uy, *readedge, uy34 | ((u64)(node & YZMASK) << ZBITS) | *readz, vx);
    ...
    rdtsc1 = __rdtsc();
    if (!id) DO_NOTHING("genVnodes round %2d size %u rdtsc: %lu\n", uorv, sumsize/BIGSIZE, rdtsc1-rdtsc0);
...
// write      UXXXXX    UYYYYY     UZZZZZ     VZZZZ   within VX VY partition
        ...
        { DO_NOTHING("OOPS3: id %d vx %d ux %d UXY %x\n", id, vx, ux, uxyz); exit(0); }
      }
      u8 *degs = tdegs[id];
      small.storeu(tbuckets+id, 0);
...
// DO_NOTHING("id %d vx %d vy %d e %010lx suffUX %02x UX %x mask %x\n", id, vx, vy, e, (u32)(e >> YZZBITS), ux, SRCPREFMASK);
...
// write     VYYYYY    VZZZZZ     UYYYYY     UZZZZ   within UX partition
        ...
        { DO_NOTHING("OOPS4: id %d vx %x ux %x vs %x\n", id, vx, ux, XMASK); }
      }
...
// if (round==32 && ux==25) DO_NOTHING("id %d vx %d ux %d e %010lx suffUXYZ %05x suffUXY %03x UXYZ %08x UXY %04x mask %x\n", id, vx, ux, e, (u32)(e >> YZBITS), (u32)(e >> YZZBITS), uxyz, uxyz>>ZBITS, SRCPREFMASK);
          const u32 vy = (e >> ZBITS) & YMASK;
...
// if (TRIMONV&&vx==75&&vy==83) DO_NOTHING("id %d vx %d vy %d e %010lx e15 %x ux %x\n", id, vx, vy, ((u64)uxyz << ZBITS) | (e & ZMASK), uxyz, uxyz>>YZBITS);
          if (TRIMONV)
            uyz &= ~ZMASK;
...
// read           VXXXXXX     VYYYZZ'     UZZZZ   within UX UY partition  if !TRIMONV
          ...
          u16 vdeg = degs[vz];
// if (TRIMONV&&vx==75&&vy==83) DO_NOTHING("id %d vx %d vy %d e %010lx e37 %x ux %x vdeg %d nrenames %d\n", id, vx, vy, e, e>>YZZBITS, ux, vdeg, nrenames);
          if (vdeg) {
            if (vdeg < 32) {
              degs[vz] = vdeg = 32 + nrenames++;
              *renames++ = vy << ZBITS | vz;
              if (renames == endrenames) {
                endrenames += (TRIMONV ? sizeof(yzbucket<ZBUCKETSIZE>) : sizeof(mean_zbucket<ZBUCKETSIZE>)) / sizeof(u32);
...
// if (vx==44&&vy==58) DO_NOTHING("  id %d vx %d vy %d newe %010lx\n", id, vx, vy, vy28 | ((vdeg) << YZBITS) | ((e >> ZBITS) & YZMASK));
    ...
    rdtsc1 = __rdtsc();
    if (showall || !id) DO_NOTHING("trimrename id %d round %2d size %u rdtsc: %lu maxnnid %d\n", id, round, sumsize/DSTSIZE, rdtsc1-rdtsc0, maxnnid);
    assert(maxnnid < NYZ1);
    tcounts[id] = sumsize/DSTSIZE;
  }
  ...
  void trimedges1(const u32 id, const u32 round) {
    ...
    for (u32 vx = startvx; vx < endvx; vx++) {
      ...
      for (u32 ux = 0 ; ux < NX; ux++) {
        ...
        // DO_NOTHING("id %d vx %d ux %d size %d\n", id, vx, ux, zb.size/SRCSIZE);
...
// read      UYYYYY    UZZZZ'     VYYYY     VZZ'   within VX partition
          ...
          // DO_NOTHING("id %d vx %d ux %d e %08lx vyz %04x uyz %04x\n", id, vx, ux, e, vyz, e >> YZ1BITS);
// bit       29..22    21..15     14..7     6..0
...
// write     VYYZZZ"     UYYZZ'   within UX partition
    ...
    rdtsc1 = __rdtsc();
    if (showall || !id) DO_NOTHING("trimrename1 id %d round %2d size %u rdtsc: %lu maxnnid %d\n", id, round, sumsize/sizeof(u32), rdtsc1-rdtsc0, maxnnid);
    assert(maxnnid < NYZ2);
    tcounts[id] = sumsize/sizeof(u32);
  }
  ...
  void trim() {
    ...
    mean_thread_ctx *threads = new mean_thread_ctx[nthreads];
    for (u32 t = 0; t < nthreads; t++) {
      threads[t].id = t;
      threads[t].et = this;
...
void *etworker(void *vp) {
  mean_thread_ctx *tp = (mean_thread_ctx *)vp;
  tp->et->trimmer(tp->id);
  pthread_exit(NULL);
  return 0;
}
...
class mean_solver_ctx;

typedef struct {
  u32 id;
  pthread_t thread;
  mean_solver_ctx *solver;
} mean_match_ctx;
static void *matchworker(void *vp);

class mean_solver_ctx {
public:
  mean_edgetrimmer *trimmer;
  u32 *cuckoo = 0;
  bool showcycle;
  proof cycleus;
  proof cyclevs;
  ...
  mean_solver_ctx(const u32 n_threads, const u32 n_trims, bool allrounds, bool show_cycle) {
    trimmer = new mean_edgetrimmer(n_threads, n_trims, allrounds);
  ...
  u32 threadbytes() const {
    return sizeof(mean_thread_ctx) + sizeof(yzbucket<TBUCKETSIZE>) + sizeof(zbucket8) + sizeof(zbucket16) + sizeof(zbucket32);
  }
  ...
  void solution(const u32 *us, u32 nu, const u32 *vs, u32 nv) {
    ...
    if (showcycle) {
      ...
      mean_match_ctx *threads = new mean_match_ctx[trimmer->nthreads];
  ...
  u32 path(u32 u, u32 *us) const {
    ...
    for (nu = 0; u != CUCKOO_NIL; u = cuckoo[u]) {
      if (nu >= MAXPATHLEN) {
        ...
        if (!~nu)
          0;
        else DO_NOTHING("illegal %4d-cycle from node %d\n", MAXPATHLEN-nu, u0);
        pthread_exit(NULL);
      }
      us[nu++] = u;
    }
  ...
  void findcycles() {
    ...
    for (u32 vx = 0; vx < NX; vx++) {
      for (u32 ux = 0 ; ux < NX; ux++) {
        mean_zbucket<ZBUCKETSIZE> &zb = trimmer->buckets[ux][vx];
        u32 *readbig = zb.words, *endreadbig = readbig + zb.size/sizeof(u32);
// DO_NOTHING("vx %d ux %d size %u\n", vx, ux, zb.size/4);
        for (; readbig < endreadbig; readbig++) {
// bit        21..11     10...0
...
// DO_NOTHING("vx %02x ux %02x e %08x uxyz %06x vxyz %06x u0 %x v0 %x nu %d nv %d\n", vx, ux, e, uxyz, vxyz, u0, v0, nu, nv);
            if (us[nu] == vs[nv]) {
  ...
  void *matchUnodes(mean_match_ctx *mc) {
    u64 rdtsc0, rdtsc1;
  ...
  #error not implemented
    ...
    rdtsc1 = __rdtsc();
    if (trimmer->showall || !mc->id) DO_NOTHING("matchUnodes id %d rdtsc: %lu\n", mc->id, rdtsc1-rdtsc0);
    pthread_exit(NULL);
    return 0;
  }
};
...
static void *matchworker(void *vp) {
  mean_match_ctx *tp = (mean_match_ctx *)vp;
